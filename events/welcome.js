    ...
    if (data) {
        ...
        if (data2) {
            ...
            let embed2 = new MessageEmbed
                .setTitle("Welcome")
                .setDescription(
                    ` <:FosterBot:1191014868267049041> ${member}, Welcome to **${member.guild.name}**! We hope you like our Server! Enjoy Your Stay here!`
                )
                .setFooter(` <:FosterBot:1191014868267049041> We are now ${member.guild.memberCount} members`)
                .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
                .setColor("GREEN");

            let channel = data.Welcome

            member.guild.channels.cache.get(channel).send(` <:FosterBot:1191014868267049041> Hey ${member} Welcome to ${member.guild.name}`)
